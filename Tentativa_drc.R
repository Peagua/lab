...
colnames(prep_total)<-c("dose", "T1_R1", "T1_R2", "T1_R3", "T1_R4")
...
prep_total %>% 
  ...
  mutate(
    condition = str_sub(sample,1,2),
    replicate = as.numeric(str_sub(sample,5))
  ) -> prep_tidy
...
predict_data %>% 
  mutate(
    condition = rep("T1", each=(nrow(predict_data)))
  )->predict_data
